...
<style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    /* Scoped badges to avoid conflicts */
    .tag-badge { display:inline-flex; align-items:center; gap:.5rem; padding:.35rem .9rem; border-radius:20px; font-size:.8rem; font-weight:600; }
    ...
    <div class="container-max">
        <div class="welcome-header">
            <div class="welcome-content">
                <div class="welcome-title">
                    <div class="header-icon"><i class="fas fa-folder"></i></div>
                    <div>
                        <h2 style="margin:0; font-size:1.6rem; font-weight:800;">Gestion des Catégories</h2>
                        <div style="opacity:.9;">Création et gestion des catégories de patrimoine</div>
                    </div>
                </div>
        ...
        <div class="card" style="margin-bottom:1.5rem;">
            <div class="card-inner">
                <div class="section-title"><i class="fas fa-plus-circle" style="color:var(--primary);"></i> Créer une nouvelle catégorie</div>
                <form action="{{ route('admin.categories.creer') }}" method="POST" class="form-grid">
                    @csrf
                    <div class="form-group">
                        <label>Initiale <span style="color:var(--danger);">*</span></label>
                        <input type="text" name="initiale" class="form-control" placeholder="ex: CPNU" required maxlength="10">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group" style="display:flex; align-items:flex-end;">
                        <button type="submit" class="btn btn-success" style="width:100%;"><i class="fas fa-save"></i> Créer</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="main-card">
            <div class="card-header">
                ...
                <span class="tag-badge tag-badge-gray"><i class="fas fa-hashtag"></i> {{ $categories->total() }} total</span>
            </div>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th><i class="fas fa-tag"></i> Initiale</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="color:var(--dark); font-weight:700;">{{ $cat->initiale }}</td>
                            <td>{{ $cat->nom_complet }}</td>
                            <td>
                                <div style="display:flex; align-items:center; gap:.5rem;">
                                    <div style="width:16px; height:16px; border-radius:4px; background-color: {{ $cat->couleur }}"></div>
                                    <span style="font-size:.85rem; color:var(--gray)">{{ $cat->couleur }}</span>
                                </div>
                            </td>
                            <td><span class="tag-badge tag-badge-gray"><i class="fas fa-user"></i> {{ $cat->patrimoines_count }}</span></td>
                            <td>
                                <div style="display:flex; gap:.5rem; justify-content:center;">
                                    @if($cat->patrimoines_count == 0)
                                        <form action="{{ route('admin.categories.supprimer', $cat) }}" method="POST" onsubmit="return confirm('Supprimer cette catégorie ?')">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-danger" style="padding:.5rem .9rem; border-radius:10px;"><i class="fas fa-trash"></i> Supprimer</button>
                                        </form>
                                    @else
                                        <span class="tag-badge tag-badge-gray"><i class="fas fa-lock"></i> Contient des patrimoines</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
            ...
            <div class="table-footer">
                <div style="color:var(--gray);"><i class="fas fa-info-circle"></i> {{ $categories->count() }} catégorie(s) affichée(s)</div>
